...
pub async fn get_youtube_videodata(url: String) -> Result<serde_json::Value, String> {
    ...
    let re_player =
        Regex::new(r"var ytInitialPlayerResponse\s*=\s*(\{.+?\});").map_err(|e| e.to_string())?;

    // Extract ytInitialData for social/channel details
    let re_data = Regex::new(r"var ytInitialData\s*=\s*(\{.+?\});").map_err(|e| e.to_string())?;

    let player_json: Option<Value> = re_player
        .captures(&html)
        .and_then(|caps| caps.get(1))
        .and_then(|m| serde_json::from_str(m.as_str()).ok());

    let initial_data: Option<Value> = re_data
        .captures(&html)
        .and_then(|caps| caps.get(1))
        .and_then(|m| serde_json::from_str(m.as_str()).ok());
            ...
            for item in contents_arr {
                if let Some(secondary) = item.get("videoSecondaryInfoRenderer") {
                    if let Some(owner) = secondary.pointer("/owner/videoOwnerRenderer") {
                        // Channel Name
                        result["channelName"] = owner
                            .pointer("/title/runs/0/text")
                            .cloned()
                            .unwrap_or(json!(""));

                        // Channel Url
                        result["channelUrl"] = owner
                            ...
                            .and_then(|t| t.as_array())
                        {
                            if let Some(last) = thumbs.last() {
                                result["channelAvatar"] =
                                    last.get("url").cloned().unwrap_or(json!(""));
                            }
                        }

                        // Subscriber Count
                        result["subscriberCount"] = owner
                ...
                if let Some(primary) = item.get("videoPrimaryInfoRenderer") {
                    ...
                    if result["publishDate"] == json!("") {
                        result["publishDate"] = primary
                            .pointer("/dateText/simpleText")
                            .cloned()
                            .unwrap_or(json!(""));
                    }
                }
